...
if [ -z "$DISPLAY" ]; then
    ...
    echo -e ""
    echo -e "${azul}   -${borra_colores} Si lo estas ejecutando en una terminal de ssh:"
    echo -e "       Tienes que utilizar la opcion (-Y) en tu conexion. ej. ssh -Y nombre_usuario@tu_ip"
    echo -e "${azul}   -${borra_colores} Tienes que tener habilitadas las opciones siguientes en tu server ssh"
    echo -e "       Que se encuentran en el fichero de configuracion en esta ruta: /etc/ssh/sshd_config"
